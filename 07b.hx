// d=$(mktemp -d) && cp 07b.hx $d/Solution.hx && haxe -C $d --main Solution --interp

using StringTools;

class Solution {
    static var cards = "J23456789TQKA";

    static function getStrength(card: String): Int {
        return cards.indexOf(card);
    }

    static function getTypeFixed(hand: String): Int {
    ...
    static function getType(hand: String): Int {
        ...
        for (card in cards.split("")) {
            var current = getTypeFixed(hand.replace("J", card));
