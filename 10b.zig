...
fn markBoundary(b: *[N][N]bool, p: Point) void {
    b[@intCast(p.x)][@intCast(p.y)] = true;
}
...
pub fn main() !void {
    ...
    var b = [_][N]bool{[_]bool{false} ** N} ** N;
    markBoundary(&b, start);
    ...
    for (0..n) |x| {
        for (0..m) |y| {
            ...
            for (y..m) |yy| {
                if (b[x][yy] and (a[x][yy] == '|' or a[x][yy] == 'L' or a[x][yy] == 'J')) inside = !inside;
            }
            if (inside) result += 1;
        }
    }
